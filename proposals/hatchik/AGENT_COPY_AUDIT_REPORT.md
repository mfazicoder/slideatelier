...
| Linear MCP / Linear-based backlog | Not wired (and Linear MCP server doesn't exist) | All Linear references in `substrate-template/CLAUDE.md` replaced with `BACKLOG.md` workflow. `mcp.json` + `.cursor/mcp.json` Linear blocks removed (Loftik kept), with a `_comment` describing when they'll come back. FAQ "How does the to-do list work?" + bundle card 10 reworked to lead with BACKLOG.md. Linear positioned as a planned Launch-tier upsell |
...
- Did NOT rewrite the MCP install instructions themselves — the Loftik MCP wasn't flagged as not-built in the brief, and the page already documents real config formats. If Loftik MCP isn't live by launch this becomes a credibility risk; flagged below.
...
- Linear MCP block removed (Loftik kept). Added a `_comment` field describing when it comes back.
...
1. **HIGH — Loftik MCP** install.html promises a wired MCP. Until that MCP ships, anyone following the install instructions will get a 404 on `@loftik/mcp`. Easiest fix: add a "private beta" banner to install.html and route everyone through the website wizard until the MCP is real.
