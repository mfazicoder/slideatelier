...
- Note on the dev workflow: push to GitHub → Hatchik redeploys (the
  redeploy daemon isn't built yet — flagged as follow-up below)
...
### Follow-up: redeploy daemon

The walkthrough email tells customers "push your changes — Hatchik
picks them up automatically". The redeploy daemon that watches GitHub
webhooks and `git pull` + `docker compose up -d` for the tenant isn't
built yet. Customers who try to push before that ships will need a
manual nudge ("I've pulled and redeployed your sandbox — give me a few
hours to ship the auto-redeploy").
