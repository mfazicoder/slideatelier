...
def provision_owner_user(
    ...
    The `first_name` (if known from the signup form) is forwarded as
    `user_metadata.full_name`, which the substrate's on-signup trigger
    copies into `public.users.display_name` and `/account` reads as the
    initial Display Name. Lets us greet customers by name from the very
    first visit without them filling another form.
    """
    ...
    trimmed = (first_name or "").strip()
    if trimmed:
        # full_name is the key the substrate trigger reads; first_name is
        # kept too so /account's display-name fallback has both options.
...
def main() -> None:
    ...
    try:
        ...
        signin_link = provision_owner_user(slug, port, email, target, first_name=first_name)
        if signin_link:
